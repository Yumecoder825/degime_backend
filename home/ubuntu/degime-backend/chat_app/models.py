...
class ChatMessage(models.Model):
    ...
    def count_overall_unread_msg(username):
        ...
        for ch_session in user_all_friends:
            un_read_msg_count = ChatMessage.objects.filter(room_name=ch_session.id, message__read=False).exclude(
                user=user).count()
    ...
    def all_msg_read(room_name, username):
        all_msg = ChatMessage.objects.filter(room_name=room_name, message__read=False).exclude(user__username=username)
        for msg in all_msg:
            msg.messge['read'] = True
